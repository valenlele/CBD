...
begin
  ...
  while (min.anio <> VALOR_ALTO) do begin
    rM.anio := min.anio;
    while (min.anio <> VALOR_ALTO) and (min.anio = rM.anio) do begin
      totVentas := 0;
      totMonto := 0;
      rM.mes := min.mes;
      while (min.anio <> VALOR_ALTO) and (min.anio = rM.anio) and (min.mes = rM.mes) do begin
        rD.dia := min.dia;
        while (min.anio <> VALOR_ALTO) and (min.anio = rM.anio) and (min.mes = rM.mes) and (min.dia = rD.dia) do begin
          marcaAct := min.marca;
          maxVentas := -1;
          minVentas := 9999;
          while (min.anio <> VALOR_ALTO) and (min.anio = rM.anio) and (min.mes = rM.mes) and (min.dia = rD.dia) and (min.marca = marcaAct) do begin
            modeloAct := min.modelo;
            totVentasModelo := 0;
            while (min.anio <> VALOR_ALTO) and (min.anio = rM.anio) and (min.mes = rM.mes) and (min.dia = rD.dia) and (min.marca = marcaAct) and (min.modelo = modeloAct) do begin
